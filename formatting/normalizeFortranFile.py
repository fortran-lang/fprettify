...
def cleanDeclarations(routine):
    ...
    if routine['core']:
        if containsRe.match(routine['core'][-1]):
            logging.error("*** routine %s contains other routines ***\n*** declarations not cleaned ***\n" %
                          (routine['name']))
            return
    ...
    if (routine['core']):
        if re.match(" *type *[a-zA-Z_]+ *$", routine['core'][0], re.IGNORECASE):
            logging.error("*** routine %s contains local types, not fully cleaned ***\n" %
                          (routine['name']))
        if re.match(" *import+ *$", routine['core'][0], re.IGNORECASE):
            logging.error("*** routine %s contains import, not fully cleaned ***\n" %
                          (routine['name']))
    if re.search("^#", "".join(routine['declarations']), re.MULTILINE):
        logging.error("*** routine %s declarations contain preprocessor directives ***\n*** declarations not cleaned ***\n" % (
            routine['name']))
        return
    try:
        ...
        for arg in routine['lowercaseArguments']:
            if argDeclDict.has_key(arg):
                ...
                logging.error("warning, implicitly typed argument '" +
